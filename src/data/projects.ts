...
export const projects: Project[] = [
  ...
  },
  {
    ...
    highlights: [
      "Multi-format Import Support: Handles JSON payloads, multipart file uploads, and streaming data with automatic content type detection",
      "Resumable Upload Implementation: TUS protocol integration with configurable chunk sizes and retry mechanisms",
      "Background Job Processing: Graphile Worker with 10 concurrent jobs and 500ms poll interval for scalable processing",
      "Data Validation Pipeline: Zod schema validation for contact data with name/email requirements and detailed error reporting",
      "Storage Management: Supabase Storage integration with automatic file cleanup after processing",
      "Error Handling System: Custom ImportError class with error codes and graceful error recovery",
      "Configuration Management: Environment validation using Zod with type-safe configuration objects and sensible defaults",
      "Testing Infrastructure: Comprehensive test suite with unit, integration, and E2E tests using Vitest",
      "Health Monitoring: Health check endpoints with database and storage connectivity validation",
      "Content Type Detection: Automatic detection of JSON, multipart/form-data, and stream inputs",
      "Worker Architecture: Dependency injection pattern with repository and service layer abstractions",
      "File Processing: Support for both regular and resumable uploads with metadata tracking",
      "API Design: RESTful endpoints with proper HTTP status codes and error responses",
      "Development Tooling: Hot reloading, Biome code formatting, and conventional commit structure",
    ],
    stack: [
      "Hono",
      "TypeScript",
